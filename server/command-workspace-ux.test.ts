...
describe("v2 task-first workspace production regressions", () => {
  ...
  it("keeps the right sidebar readable by making files primary and advanced tools collapsed", () => {
    ...
    expect(source).toContain("Upload to Global Files");
  ...
  it("wires the page to current v2 tRPC procedures rather than removed workspace and command routers", () => {
    ...
    expect(source).toContain("trpc.files.listGlobalForTask.useQuery");
    expect(source).toContain("trpc.files.attachGlobalToTask.useMutation");
  ...
  it("keeps honest empty states instead of demo records for files and memory", () => {
    ...
    expect(source).toContain("Global Files is empty");
